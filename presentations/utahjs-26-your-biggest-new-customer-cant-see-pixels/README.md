...
pnpm --filter @supaslidev/utahjs-26-your-biggest-new-customer-cant-see-pixels dev      # dev server
pnpm --filter @supaslidev/utahjs-26-your-biggest-new-customer-cant-see-pixels build    # static SPA (canonical artifact)
pnpm --filter @supaslidev/utahjs-26-your-biggest-new-customer-cant-see-pixels export   # PDF (degraded: demos become static frames)
```

Build with a base path for hosting under talks.timdamen.io:

```bash
pnpm --filter @supaslidev/utahjs-26-your-biggest-new-customer-cant-see-pixels build -- --base /presentations/utahjs-26-your-biggest-new-customer-cant-see-pixels/
```
...
1. Confirm UtahJS 2026 date + exact slot length (deck assumes 40 min).
...
8. Re-verify Adobe/Salesforce/OpenAI numbers ~2 weeks before the conference
   (see SOURCES.md).
9. Bio/headshot slide wanted? Currently omitted — check whether UtahJS
   intros speakers.
