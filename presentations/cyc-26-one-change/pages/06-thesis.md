# What this actually bought

<div class="bought">
<v-clicks>

<p>One script, once a sprint, and an app is update with the latest greatest again.</p>
<p>A breaking design system major shipped to every app in two weeks.</p>
<p>Shared and aligned frontend stack over among the organisation.</p>
<p></p>

</v-clicks>
</div>

<!--
⏱ 27:10 — Act 6. The thesis. Four lines, no adjectives, let them land.

[click] One script, run about once a sprint. That is the whole interface a feature team ever sees, and it is the reason nobody on one had to read a release note this year.

[click] We met the accessibility deadline. I want to be precise about how: nobody on a feature team edited a test file to do it. The import was already ours.

[click] A design system major went to every application in two weeks. I have done that same job the other way, and it took the better part of a year.

[click] Four upstream majors in two years. There are five of us. That is the number I'd put in front of anyone asking what a platform team is for.
-->

---
layout: center
---

<div class="thesis">
  <p class="thesis-line">Feature teams ship what the customer asked for.</p>
  <p class="thesis-line accent" v-click>Platform teams ship the thing <span class="underline-accent">nobody asked for but everybody needs.</span></p>
  <p class="thesis-sub" v-click>You cannot mandate it. You can only make the platform so good that people want to use it.</p>
</div>

<!--
⏱ 27:40

Here's the argument I actually came to make.

Feature teams are measured in customer value, and they should be. Platform teams are measured in nothing visible at all, right up until the moment something breaks, and then they're measured in that.

[click] We ship the thing nobody asked for and everybody needs. Governance, security, performance, accessibility. These arrive from outside engineering, they are not negotiable, and they land on people whose job is something else entirely.

[click] And the constraint that shapes everything: I have no authority. Not a single one of those three hundred teams reports to me. I cannot mandate a thing. All I can do is make the paved road so obviously better than the alternative that stepping off it feels like work.

That's what all of this is. The one-line script, the four commands, the frozen helper library, the two hundred and three lines that never call a printer. It's all one idea: make the correct thing the cheap thing, and then never, ever betray the people who trusted you with write access.
-->

---
layout: default
---

<OutroSlide title="Thank you" />

<!--
⏱ 29:20 — Last line. Say it and stop.

Every demo in this talk parsed real code, in the browser, while you watched. There are no screenshots in this deck and no recorded terminals. If it looked like it computed something, it computed it.

Thank you.

[Q&A buffer to 30:00]
-->
