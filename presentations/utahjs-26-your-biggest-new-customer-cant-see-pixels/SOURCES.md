...
12. **Harnesses agents drive browsers with** (slide 8) — verified Aug 2026:
    ...
    - **Playwright MCP** (Microsoft) — accessibility snapshot is the default,
      vision is opt-in (already cited in §8). README (checked 16 Aug 2026):
      *"Uses Playwright's accessibility tree, not pixel-based input"*, *"no
      vision models needed"*; coordinate tools are opt-in via `--caps=vision`.
